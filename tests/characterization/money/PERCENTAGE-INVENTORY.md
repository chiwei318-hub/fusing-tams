...
| **80%** | AP / driver payable **REMOVED** (#3F) | `financials.ts` trigger | was `*0.80` → now **NULL** | `order_financials.ap_total` | delivered first insert | — | NULL | N | N | Y NULL UI | Financials「待計算」 | **#3F REPAIR Done** |
| **80%** | Writer B fallback **REMOVED** (#3F) | `calcFinancials` / `arApLedger` | was `driver_payout<=0 → *0.80` → now verified settlement or NULL | `ap_total` | recalc | settlement | — | N | N | Y | Financials / ArAp | **#3F**; collision naming OPEN |
| **500** | Tailgate add-on on AP path | `financials.ts` / `arApLedger.ts` | if `need_tailgate` | `ap_tailgate` | Writer B / ar-ap | fixed | literal 500 | Y | N (≠ pricing.ts 800) | Y | AP total | **UNVERIFIED_DEFAULT** |
| **800** | Hydraulic add-on on AP path | same | if `need_hydraulic_pallet` | `ap_other_equipment` | Writer B / ar-ap | fixed | literal 800 | Y | N (≠ pricing.ts 600) | Y | AP total | **UNVERIFIED_DEFAULT** |
| **85%** | Implied driver share via settlement GENERATED | `schema/settlements.ts` | `driver_payout` GENERATED | `order_settlements.driver_payout` | when settlement row exists | `total_amount` | `(100−commission_rate)%` (default rate 15) | Y default | Y rate | Y | Writer B preferred AP base | **≠ 80% fallback** |
